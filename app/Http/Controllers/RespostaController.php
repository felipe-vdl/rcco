      ...
      foreach ($request->topicos as $chave => $topico) {
        ...
        if (isset($topico["imagens"])) {
          foreach ($topico["imagens"] as $chave => $imagem) {
            if (!Resposta::where([['data', $request->data], ['user_id', Auth::user()->id]])
              ->where('unidade_id', $request->unidade_id)
              ->where('pergunta_id', $imagem["pergunta_id"])
              ->exists()) {
              $resposta = new Resposta;
              $resposta->data = $request->data;
              $resposta->unidade_id = $request->unidade_id;
              $resposta->valor = "";
              $resposta->status = 0;
              $resposta->pergunta_id = $imagem["pergunta_id"];
              $resposta->topico_id = $imagem["topico_id"];
              $resposta->user_id = Auth::user()->id;
              if (isset($request->marcador_id)) {
                $resposta->marcador_id = $request->marcador_id;
              }
        ...
        if (isset($topico["videos"])) {
          foreach ($topico["videos"] as $chave => $video) {
            if (!Resposta::where([['data', $request->data], ['user_id', Auth::user()->id]])
              ->where('unidade_id', $request->unidade_id)
              ->where('pergunta_id', $video["pergunta_id"])
              ->exists()) {
              $resposta = new Resposta;
              $resposta->data = $request->data;
              $resposta->unidade_id = $request->unidade_id;
              $resposta->valor = "";
              $resposta->status = 0;
              $resposta->pergunta_id = $video["pergunta_id"];
              $resposta->topico_id = $video["topico_id"];
              $resposta->user_id = Auth::user()->id;
              if (isset($request->marcador_id)) {
                $resposta->marcador_id = $request->marcador_id;
              }
        ...
        if (isset($topico["documentos"])) {
          foreach ($topico["documentos"] as $chave => $documento) {
            if (!Resposta::where([['data', $request->data], ['user_id', Auth::user()->id]])
              ->where('unidade_id', $request->unidade_id)
              ->where('pergunta_id', $documento["pergunta_id"])
              ->exists()) {
              $resposta = new Resposta;
              $resposta->data = $request->data;
              $resposta->unidade_id = $request->unidade_id;
              $resposta->valor = "";
              $resposta->status = 0;
              $resposta->pergunta_id = $documento["pergunta_id"];
              $resposta->topico_id = $documento["topico_id"];
              $resposta->user_id = Auth::user()->id;
              if (isset($request->marcador_id)) {
                $resposta->marcador_id = $request->marcador_id;
              }
